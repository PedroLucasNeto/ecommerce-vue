<template>
  <form
    ref="formProduct"
    class="flex flex-col shadow-xl space-y-4 p-4 shadow rounded-lg w-fit bg-base-100 py-4"
  >
    <div v-if="product.id">
      <label>Id</label>
      <input
    ...
    <button
      v-if="!product.id"
      class="btn btn-success w-fit text-white"
      @click.prevent="saveProduct"
    >
      Salvar
    </button>
    <button v-else class="btn btn-success w-fit text-white" @click.prevent="editProduct">
      Alterar
    </button>
  </form>
</template>
